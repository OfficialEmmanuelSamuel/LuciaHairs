...
function calculateDiff(target) {
  ...
  return {
    ...
    hours: Math.floor((sec % (60 * 60 * 24)) / 3600),
...
function TimeBlock({ value, label }) {
  return (
    <div className="flex flex-col items-center px-2">
      <span className="bg-linear-to-r from-pink-950 to-rose-950 bg-clip-text text-transparent">
        {label}
      </span>
...
export default function PreorderSection({ start, end, onPreorder, onNotify }) {
  const startDate = new Date(start || "2026-01-10T10:00:00");
  const endDate = new Date(end || "2026-02-27T24:00:00");
  ...
  // ================= Fetch Preorder Items from Backend =================
  const [items, setItems] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  useEffect(() => {
    fetch(`${import.meta.env.VITE_API_BASE_URL}/preorders`)
      .then((res) => {
        if (!res.ok) throw new Error("Failed to load preorder items");
        return res.json();
      })
      .then(setItems)
  ...
  return (
    <section id="preorder" className="relative overflow-hidden">
      ...
      <p className="text-xl mt-25 p-2 mb-8 text-center font-quicksand font-medium bg-linear-to-r from-gray-900 to-rose-500 bg-clip-text text-transparent">
      ...
      <div className="mx-auto max-w-9xl sm:px-2 lg:px-5 py-10 sm:py-14">
        ...
        >
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
              <div className="flex items-center gap-2 text-white/90">
                ...
                <p className="text-lg tracking-wide font-quicksand font-bold bg-linear-to-r from-pink-900 to-rose-900 bg-clip-text text-transparent">
              ...
              <p className="text-gray-900 w-70 font-quicksand font-medium mt-1 shadow-sm shadow-gray-500 rounded-full py-2 text-center">
                {subline}
              </p>
            </div>
            ...
            <div className="flex items-center justify-center">
              {phase !== "after" ? (
                ...
                <div className="flex items-center gap-2 bg-linear-to-r from-pink-900 to-rose-900 bg-clip-text text-transparent">
                  ...
                  <span className="font-semibold">Thanks! We’ll reopen soon.</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
            ...
            {error && (
              <p className="text-center text-red-600 font-semibold col-span-full">{error}</p>
            )}
